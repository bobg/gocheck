...
import (
	...
	"github.com/bobg/gocheck/internal/analysisflags"
	"github.com/bobg/gocheck/internal/diff"
	"github.com/bobg/gocheck/internal/robustio"
)

type Controller struct {
	Show    bool // show facts as they are created
	Serial  bool // run serially (i.e., disable parallel execution of analyzers)
	Sanity  bool // do additional sanity checks on fact types and serialization)
	Timing  bool // show timing info (use Serial=true to avoid GC/scheduler noise)
	Fix     bool // apply suggested fixes
	Verbose bool // verbose logging
	JSON    bool // emit JSON diagnostics
}

const (
	LoadMode     = packages.NeedName | packages.NeedFiles | packages.NeedCompiledGoFiles | packages.NeedImports | packages.NeedTypes | packages.NeedTypesSizes | packages.NeedSyntax | packages.NeedTypesInfo
	LoadDepsMode = LoadMode | packages.NeedDeps
)

func (c *Controller) Run(pkgs []*packages.Package, analyzers []*analysis.Analyzer) ([]analysis.Diagnostic, error) {
	// Analyze the packages.
	roots := c.analyze(pkgs, analyzers)
...
func (c *Controller) analyze(pkgs []*packages.Package, analyzers []*analysis.Analyzer) []*action {
...
func (c *Controller) printDiagnostics(roots []*action) (exitcode int) {
	// Print the output.
	//
...
func (c *Controller) execAll(actions []*action) {
	sequential := c.Serial
